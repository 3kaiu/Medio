...
impl MediaProbe for NativeProbe {
    fn probe(&self, path: &Path) -> Result<QualityInfo, Box<dyn std::error::Error>> {
        ...
        let mut info = match ext.as_str() {
            "mp4" | "m4a" | "m4v" | "mov" => self.probe_mp4(path).unwrap_or_else(|_| QualityInfo::new(ProbeSource::Native)),
            "mkv" => self.probe_mkv(path).unwrap_or_else(|_| QualityInfo::new(ProbeSource::Native)),
            "mp3" | "flac" | "ogg" | "wav" | "opus" => self.probe_audio(path).unwrap_or_else(|_| QualityInfo::new(ProbeSource::Native)),
                        ...
                        for entry in stsd.descriptions {
                            match entry {
                                mp4parse::SampleEntry::Audio(a) => {
                                    ...
                                    info.audio_bitrate = Some(a.samplerate as u64 * a.channelcount as u64 * 16);
                                }
                                _ => {}
                            }
                        }
        ...
        if let Ok(file) = std::fs::File::open(path) {
            ...
            if let Ok(probed) = symphonia::default::get_probe().format(
                ...
                if let Some(track) = probed.format.default_track() {
                    ...
                    info.duration_secs = params.time_base.and_then(|tb| {
                        params.n_frames.map(|f| tb.calc_time(f).seconds as u64)
                    });
                }
            }
        }
