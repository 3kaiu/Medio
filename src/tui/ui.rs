...
fn draw_content(f: &mut Frame, app: &App, area: Rect) {
    ...
    let (rows, header, widths, title) = match app.tab {
        ...
        Tab::Dedup => {
            ...
            let rows: Vec<Row> = filtered.iter().map(|(_, group)| {
            ...
            (
                ...
                vec![
                    Constraint::Length(35), Constraint::Length(8), Constraint::Length(8),
        ...
        Tab::Rename => {
            ...
            (
                ...
                vec![Constraint::Length(29), Constraint::Length(29), Constraint::Length(6)],
        ...
        Tab::Organize => {
            ...
            (
                ...
                vec![
                    Constraint::Length(10), Constraint::Length(25), Constraint::Length(29),
    ...
    let header = header.style(Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD).bg(Color::DarkGray));
    ...
    let detail = match app.tab {
        ...
        Tab::Dedup => {
            ...
            if app.selected < filtered.len() {
                ...
                lines
            } else { vec![Line::from("No group selected")] }
        }
        Tab::Rename => {
            let filtered = app.filtered_rename_plans();
            if app.selected < filtered.len() {
                ...
                for sub in &plan.subtitle_plans {
                    lines.push(Line::from(format!("Sub: {} -> {}", sub.old_path.display(), sub.new_path.display())));
                }
                lines
            } else { vec![Line::from("No plan selected")] }
        }
        Tab::Organize => {
            let filtered = app.filtered_organize_plans();
            if app.selected < filtered.len() {
                ...
                vec![
                    ...
                    Line::from(format!("NFO: {}", if plan.nfo_content.is_some() { "yes" } else { "no" })),
